...
| **`claude-md-map`** | Reviews or (re)writes a CLAUDE.md/AGENTS.md so it works as a map — a short entry point that says where to look — not an encyclopedia. Grades 7 pass/fail criteria and restructures. Ships `scripts/claude_md_lint.sh` (≤120 lines, relative links resolve), installable as a PostToolUse hook. |
